...
int	mpl3115ConfigMode (struct rule_t * psR, int Xcur, int Xmax, int EI) {
	...
	IF_PRINT(debugCONFIG && ioB1GET(ioMode), "MODE 'MPL3115' Xcur=%d Xmax=%d mode=%d os=%d step=%d\n", Xcur, Xmax, mode, os, step);
...
void mpl3115ReportAll(void) {
	...
	PRINT("\tDR_STATUS: 0x%02X  PTOW=%d  POW=%d  TOW=%d  PTDR=%d  PDR=%d  TDR=%d\n", sMPL3115.Reg.DR_STATUS,
	...
	PRINT("\tF_STATUS: 0x%02X  OVF=%d  WMRK=%d  CNT=%d\n", sMPL3115.Reg.F_STATUS,
	...
	PRINT("\tF_SETUP: 0x%02X  MODE=%d  WMRK=%d\n", sMPL3115.Reg.F_SETUP,
	...
	PRINT("\tINT_SOURCE: 0x%02X\n", sMPL3115.Reg.INT_SOURCE);
	PRINT("\tPT_DATA_CFG: 0x%02X  DREM=%d  PDEFE=%d  TDEFE=%d\n", sMPL3115.Reg.PT_DATA_CFG,
	...
	PRINT("\tCTRL_REG1: 0x%02X  ALT=%d  OS=%d  RST=%d  OST=%d  SBYB=%d\n", sMPL3115.Reg.CTRL_REG1,
